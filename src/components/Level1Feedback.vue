      ...
      <div class="flex gap-2 pb-4 h-fit items-end ">
        ...
        <div class="rounded-xl  bg-slate-100 p-2 border-2 relative cursor-pointer " @click="toggleTooltip(index)"
          ...
          <div v-if="tooltipVisible === index"
               class="w-56 rounded-xl border-2 absolute transform -translate-y-1/2 top-1/2 translate-x-10 "
          >
            ...
            <div class="flex flex-col gap-1 p-2 bg-white rounded-b-xl">
              <button @click="addFeedback(key,1 )" class="primaryButton !bg-red-500 hover:!bg-red-600 w-full">A lot
                Lower
              </button>
              <button @click="addFeedback(key,2 )" class="primaryButton !bg-yellow-500 hover:!bg-yellow-600 w-full">
                Somewhat Lower
              </button>
              <button @click="addFeedback(key,3 )" class="primaryButton !bg-gray-500 hover:!bg-gray-600 w-full">About
                what it was
              </button>
              <button @click="addFeedback(key,4 )" class="primaryButton w-full">Somewhat Higher</button>
              <button @click="addFeedback(key,5 )" class="primaryButton !bg-green-500 hover:!bg-green-600 w-full">A lot
                Higher
              </button>
            </div>
          </div>
...
export default {
  ...
  setup(props,{emit}) {
    ...
    // THIS SHOULD BE A REF MAYBE TO MAKE SURE IT CHANGES WHEN THE EVALUATE NEXT ROUND IS CLICKED
/*
    const userWordsFromStorage = JSON.parse(localStorage.getItem(`roundData${props.roundNumber}`)) || {};
*/
